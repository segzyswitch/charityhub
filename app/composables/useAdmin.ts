...
export function useAdmin() {
  ...
  // 🔹 Save or update contestant
  const saveCause = async (cause: CauseData, id?: string) => {
    loading.value = true
    error.value = null
    ...
    try {
      ...
      if (!id) {
        // New contestant → image required
        if (!cause.banner || !(cause.banner instanceof File)) {
          throw new Error("Image is required when adding new cause")
        }
        imageUrl = await uploadImage(cause.banner)
      } else {
        // Updating contestant
        if (cause.banner && cause.banner instanceof File) {
          imageUrl = await uploadImage(cause.banner)
        } else {
          const oldDoc = await getDoc(doc($db, "causes", id))
          imageUrl = oldDoc.exists() ? oldDoc.data().banner : null
        }
      }
      ...
      const causeRecord = {
        ...
        datetime: cause.datetime,
        details: cause.details,
        tags: ['health', 'education', 'hunger', 'children'],
        banner: imageUrl
      }

      if (id) {
        await setDoc(doc($db, "causes", id), causeRecord, { merge: true })
        return { success: true, id }
      } else {
      ...
      error.value = err.message || "Failed to save cause"
      return { success: false, error: error.value }
    } finally {
      loading.value = false
    }
    ...
    try {
      ...
      if (!id) {
        ...
        if (method.icon && method.icon instanceof File) {
          ...
          const oldDoc = await getDoc(doc($db, "charity_payment_methods", id))
      ...
      if (id) {
        await setDoc(doc($db, "charity_payment_methods", id), methodRecord, { merge: true })
        return { success: true, id }
      } else {
        const docRef = await addDoc(collection($db, "charity_payment_methods"), {
          ...methodRecord,
          createdAt: serverTimestamp(),
        })
        return { success: true, id: docRef.id }
  ...
  const deleteCause = async (id: string) => {
    ...
    try {
      ...
      error.value = err.message || "Failed to delete cause"
      return { success: false, error: error.value }
    } finally {
      loading.value = false
    }
  ...
  const deletePaymentMethod = async (id: string) => {
    ...
    try {
      await deleteDoc(doc($db, "charity_payment_methods", id))
  ...
  return { saveContestant, saveCause, savePaymentMethod, deleteContestant, deleteCause, deleteDonation, deletePaymentMethod, deleteVote, loading, error }
}
